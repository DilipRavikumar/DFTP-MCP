...
# Scope validation for Order Details Agent
ALLOWED_SCOPES = ["MutualFunds", "Assets", "General"]
...
def process_request(request: str) -> str:
    ...
    print(f"Order Details Agent - Received request (first 200 chars): {request[:200]}")
    
    # Extract scope from request
    scope, actual_request = extract_scope_from_request(request)
    
    # Debug: Log extracted scope and request
    print(f"Order Details Agent - Extracted scope: '{scope}', actual_request: '{actual_request[:100] if len(actual_request) > 100 else actual_request}'")
    
    # Validate scope
    if not validate_scope(scope):
        error_msg = f"Unauthorized: Scope '{scope}' is not allowed for Order Details Agent. Required scopes: {', '.join(ALLOWED_SCOPES)}"
        print(f"Order Details Agent - {error_msg}")
        return error_msg
    ...
    mcp_tools = []
    try:
        # Create fresh client for each request to ensure connection health
    ...
    if mcp_tools:
        print(f"Order Details Agent: Loaded {len(mcp_tools)} MCP tools")
    else:
        print("Order Details Agent: Warning - No MCP tools loaded. MCP server may not be running on port 8002.")
    ...
    try:
        print(f"Order Details Agent - Invoking agent with request: '{actual_request[:100] if len(actual_request) > 100 else actual_request}'")
        final_state = agent.invoke(init_state)
        
        print(f"Order Details Agent - Agent execution completed. Final state keys: {list(final_state.keys())}")
            ...
            if isinstance(result, dict) and "error" in result:
                ...
                try:
                    if isinstance(result, str):
                        ...
                        try:
                            parsed = json.loads(result)
                            parts.append(f"Result: {json.dumps(parsed, indent=2)}")
                        except:
                            parts.append(f"Result: {result}")
                    else:
                        parts.append(f"Result: {json.dumps(result, indent=2)}")
                except Exception as e:
                    parts.append(f"Result: {str(result)}")
            
        response = "\n".join(parts) if parts else "Processed request (no details returned)."
        print(f"Order Details Agent - Returning response (first 200 chars): {response[:200]}")
        return response
        
    except Exception as e:
        error_msg = f"Agent execution error: {str(e)}"
        print(f"Order Details Agent Error: {error_msg}")
