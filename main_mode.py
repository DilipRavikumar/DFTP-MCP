    ...
    class ModeSelector:
        ...
        async def handle_mode_2(self, prompt: str) -> Dict[str, Any]:
            print(f"[MODE 2] Multi-step workflow: {prompt}")
            return await execute_multi_stage_workflow(prompt, self.mounted_servers, self.server_configs, self.config)
        
        async def route(self, prompt: str) -> Any:
            mode = await self.analyze_prompt(prompt)
            return await self.handle_mode_2(prompt) if mode == "mode_2" else await self.handle_mode_1(prompt)
